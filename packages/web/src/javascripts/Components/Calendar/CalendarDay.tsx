...
const CalendarDay: FunctionComponent<Props> = ({ day, activities = [], isToday, onClick, mode }) => {
  ...
  const todayClassNames = 'text-info-contrast bg-accessory-tint-7'
  const hasActivity = day > 0 && activities.length > 0
  const hasActivityClassNames = 'bg-accessory-tint-7-contrast text-accessory-tint-7 font-bold'

  return (
    <div className="h-9 w-[14.2%] p-0.5">
      <div
        className={`flex h-full w-full cursor-pointer items-center justify-center rounded ${
          hasActivity ? hasActivityClassNames : ''
        } ${isToday ? todayClassNames : ''}`}
